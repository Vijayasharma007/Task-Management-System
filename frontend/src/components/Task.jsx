...
const Task = () => {
    ...
    const completeTask=async(id)=>{
        ...
        setTasks([...tasks]) // Optimistic UI update for instant feedback!
        await instance.put(ENDPOINTS.UPDATE_TASK(id),task)
        .then(res=>{
            toast.success("Task updated successfully")
            getAllTasks()
    ...
    const getAllTasks=async()=>{
        ...
        .catch(error=>{ console.log(error); setTasks([]); })
    }
            ...
            <div className='task-container'>
                {
                    tasks && tasks.map((item,index)=>{
                        return (
                            <div key={item.id} className={`task-items ${item.completed ? 'completed-task' : ''}`}>
                                <div className='task-title'>
                                    ...
                                    <Check size={20} className='check-icon' onClick={()=>completeTask(item.id)}/>
                                </div>
